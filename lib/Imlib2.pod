...
=head3 C<context_set_dither_mask>
...
=head3 C<context_get_dither_mask>
...
=head3 C<context_set_anti_alias>
...
=head3 C<context_get_anti_alias>
...
=head3 C<context_set_mask_alpha_threshold>
...
=head3 C<context_get_mask_alpha_threshold>
...
=head3 C<context_get_color_modifier>
...
=head3 C<context_set_operation>

Usage:

	use Imlib2;
	my $im = Imlib2.new();
	$im.context_set_operation(OP_COPY);
...
=head3 C<context_get_operation>

Usage:

	use Imlib2;
	my $im = Imlib2.new();
	say "OP_ADD Operation mode" if $im.context_get_operation() == OP_ADD;
...
=head3 C<context_set_direction>

Usage:

	use Imlib2;
	my $im = Imlib2.new();
	$im.context_set_direction(TEXT_TO_RIGHT);
...
=head3 C<context_get_color_hsva>
...
=head3 C<context_get_color_hlsa>
...
=head3 C<context_get_color_cmya>
...
Usage:
	...
	my $im = Imlib2.new();
	my %cmya_channels;
	$im.context_get_color_cmya(%cmya_channels);
...
=head3 C<context_get_color_range>
...
Usage:
	...
	my $im = Imlib2.new();
	my %cliprect;
	$im.context_get_cliprect(%cliprect);
	say "x: " ~ %cliprect{'x'};
	say "y: " ~ %cliprect{'y'};
	say "width: " ~ %cliprect{'width'};
	say "height: " ~ %cliprect{'height'};

Parameters:

	%cliprect - Hash (passed as argument) with dimensions.
...
%cliprect{'x'} - The top left x coordinate of the rectangle.

=item *

%cliprect{'y'} - The top left y coordinate of the rectangle.

=item *

%cliprect{'width'} - The width of the rectangle.

=item *

%cliprect{'height'} - The height of the rectangle.

=back

=head3 C<set_cache_size>
...
Usage:
	...
	if $error_return == LOAD_ERROR_NONE {
		say "No error...";
		$image.context_set();
		... some operations ...
		$im.free_image();
...
%size{'width'} - The width in pixels of the current image.

=item *

%size{'height'} - The height in pixels of the current image.

=back

=head3 C<image_get_filename>
...
=head3 C<image_set_changes_on_disk>
...
Usage:
	...
	my $image = $im.create_image(100, 200);
	$image.context_set();
	my Imlib2::Border $border = $im.new_border();
	... some operations ...
	$im.free_image();

Returns

	Imlib2::Border $border - Struct to store the values ​​of the border.

Returns a new structure to store the values of the border of the
current context image.

=head3 C<image_set_border>
...
Parameters:
	...
	Imlib2::Border $border - The border of the image.
...
$border.left(2) - Set the value of the left side to 2.

=item *

$border.right(2) - Set the value of the right side to 2.

=item *

$border.top(4) - Set the value of the top side to 4.

=item *

$border.bottom(4) - Set the value of the bottom side to 4.

=back

=head3 C<image_get_border>
...
Usage:
	...
	say "Left: " ~ $border.left;
	say "Right: " ~ $border.right;
	say "Top: " ~ $border.top;
	say "Bottom: " ~ $border.bottom;
	... some operations ...
	$im.free_image();
...
$border.left - Returns the value of the left side.

=item *

$border.right - Returns the value of the right side.

=item *

$border.top - Returns the value of the top side.

=item *

$border.bottom - Returns the value of the bottom side.

=back

=head3 C<image_set_format>
...
=head3 C<image_set_irrelevant_format>
...
Usage:
	...
	$im.image_set_irrelevant_format($irrelevant);

Parameters:

	Bool $irrelevant - Irrelevant format flag (True or False).

Sets if the format value of the current image is irrelevant for caching
purposes - by default it is. pass irrelevant as True to make it
irrelevant and False to make it relevant for caching.

=head3 C<image_set_irrelevant_border>
...
Usage:
	...
	my $im = Imlib2.new();
	my Bool $irrelevant = True;
	$im.image_set_irrelevant_border($irrelevant);

Parameters:

	Bool $irrelevant - Irrelevant border flag (True or False).
	
Sets if the border of the current image is irrelevant for caching
purposes. By default it is. Set irrelevant to True to make it
irrelevant, and False to make it relevant.

=head3 C<image_set_irrelevant_alpha>
...
Usage:
	...
	my $im = Imlib2.new();
	my Bool $irrelevant = True;
	$im.image_set_irrelevant_alpha($irrelevant);

Parameters:

	Bool $irrelevant - Irrelevant alpha flag (True or False).

Sets if the alpha channel status of the current image (i.e. if there is
or is not one) is important for caching purposes. By default it is not.
Set irrelevant to True to make it irrelevant and False to make it
relevant.
...
=head3 C<blend_image_onto_image>
...
Usage:
	...
	my $image = $im.load_image("pictures/holidays.jpeg");
	$image.context_set();
	my FlipMode $flip_mode = FLIP_HORIZONTAL;
...
FLIP_HORIZONTAL - Flips/mirrors the current image horizontally

=item *

FLIP_VERTICAL - Flips/mirrors the current image vertically.

=item *

FLIP_DIAGONAL - Flips/mirrors the current image diagonally (good for
                quick and dirty 90 degree rotations if used before to
                after a horizontal or vertical flip).

=back

=head3 C<image_orientate>
...
Usage:
	...
	my $image = $im.load_image("pictures/holidays.jpeg");
	$image.context_set();
	my RotationMode $orientation = ROTATE_90_DEGREES;
...
ROTATE_90_DEGREES - rotates clockwise by 90 degrees.

=item *

ROTATE_180_DEGREES - rotates clockwise by 180 degrees.

=item *

ROTATE_270_DEGREES - rotates clockwise by 270 degrees.
...
Usage:
	...
	my $image = $im.load_image("pictures/holidays.jpeg");
	$image.context_set();
	my TileMode $flip_mode = TILE_HORIZONTAL;
...
TILE_HORIZONTAL - Drawn multiple times horizontally.

=item *

TILE_VERTICAL - Drawn multiple times vertically.

=item *

TILE_BOTH - Drawn multiple times horizontally and vertically.
...
Usage:
	...
	my Int $width = 0;
	my Int $height = 0;
	$im.get_text_size($text, $width, $height);
...
Usage:
	...
	$corefont.context_set();
	my Str $text = "perl6";
	my Int $horizontal_advance = 0;
	my Int $vertical_advance = 0;
	$im.get_text_advance($text, $horizontal_advance, $vertical_advance);
...
=head3 C<get_maximum_font_ascent>
...
=head3 C<get_maximum_font_descent>
...
=head3 C<remove_path_from_font_path>
...
=head3 C<image_draw_circumference>
...
=head3 C<add_color_to_color_range>
...
Usage:
	...
	say "No error..." if $error_return == LOAD_ERROR_NONE;
	$im.free_image();
